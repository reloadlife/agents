...
- **UX polish pass** (incremental, not a redesign)
  - Toasts: kind icons, dismiss button, longer hold for errors
  - Loading/empty/error states unified with spinner + retry actions
  - Stage pages animate in on open; autofocus first control
  - Copy buttons flash “Copied”; busy submit buttons show spinner
  - Sidebar empty session list, status bar, remote cards subtle polish
